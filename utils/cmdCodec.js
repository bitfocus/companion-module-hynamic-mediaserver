...
exports.decodeControlProtocol = (data) => {
	...
	// 获取业务TLV的Data字节数组
	const valBuffer = totalBuf.slice(tlvStartFind + 4, tlvDataLength + tlvStartFind + 4)
	// 返回响应的tag及数据buffer
	const result = {
		tag,
		data: valBuffer,
...
exports.encodeControlProtocol = (data) => {
	...
	const HEAD = [...PROTOCOL_HEAD, ...PACKET_TYPE, ...PROTOCOL_VERSION, ...SEQUENCE]
	...
	if (data.dataLen > 0) {
		if (data.dataType === 'hex') {
			dataBuffer = data.data.length === data.dataLen ? data.data : padArrayLittleEndian(data.data, data.dataLen)
		} else {
